...
const AntiGravityCanvas: React.FC = () => {
    ...
    const animate = useCallback((time: number) => {
        ...
        // Phase 2: Selective Collision (Optimized for Hero Background)
        // Only run collisions for a subset of particles or if count is manageable
        const COLLISION_LIMIT = 150;
        if (particles.length < COLLISION_LIMIT) {
            for (let i = 0; i < particles.length; i++) {
    ...
    return (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-none px-4">
            <div className="max-w-4xl w-full text-center space-y-8">
                ...
                >
                    <span className="py-1 px-3 border border-[#8AB4F8]/20 rounded-full text-[10px] font-mono text-[#8AB4F8]/60 tracking-[0.4em] uppercase bg-[#8AB4F8]/5 backdrop-blur-sm">
                        Academic Environment Online
                    </span>
                </motion.div>
                ...
                <motion.h1
                    ...
                    className="text-8xl md:text-9xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-white/20 tracking-tighter leading-none"
                >
                    SPIRIT<br />ACADEMY
                </motion.h1>
                ...
                <motion.p
                    ...
                    className="max-w-xl mx-auto text-sm md:text-base text-white/40 font-mono uppercase tracking-widest leading-relaxed"
                >
                    Future-proofing human intelligence. The definitive learning environment for the post-AGI era.
                </motion.p>
                ...
                >
                    <button className="group relative inline-flex items-center gap-3 px-10 py-4 bg-white text-black rounded-none font-black uppercase tracking-[0.3em] overflow-hidden transition-transform hover:scale-105 active:scale-95 text-[10px]">
                        Start Learning
                        <ArrowRight className="w-3 h-3 transition-transform group-hover:translate-x-1" />
                        <div className="absolute inset-0 bg-[#8AB4F8] transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-300 ease-out opacity-20"></div>
                    </button>
                </motion.div>
            </div>
        </div>
...
export default function ParticleHero() {
    return (
        <div className="relative w-full h-screen bg-black overflow-hidden selection:bg-[#8AB4F8] selection:text-white">
            <AntiGravityCanvas />
            <HeroContent />
