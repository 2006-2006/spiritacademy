...
export const ProfileSelector = ({
    ...
    return (
        <div
            className={cn(
                "flex min-h-screen w-full flex-col items-center justify-center bg-[#0a0a12] p-4",
                className
            )}
        >
                    ...
                    {profiles.map((profile) => (
                        <div key={profile.id} className="flex flex-col items-center gap-3 group">
                            <button
                                ...
                                className="group relative h-28 w-28 rounded-full transition-transform duration-300 ease-in-out hover:-translate-y-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#8AB4F8] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a12] md:h-36 md:w-36"
                            >
                                <div className="absolute inset-0 rounded-full bg-[#1a1a2e] transition-all duration-300 group-hover:shadow-lg group-hover:shadow-[#8AB4F8]/20"></div>
                            ...
                            <p className="text-lg text-white/60 transition-colors group-hover:text-white">
                                {profile.label}
                            </p>
                        </div>
                    ))}
...
// Helper component for non-image icons like the 'Add' button
